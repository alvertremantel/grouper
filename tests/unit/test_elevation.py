"""Tests for installer/elevation.py."""
...
class TestIsElevated:
    ...
    def test_calls_windows_api_returns_true(self) -> None:
        ...
        with patch("installer.elevation.ctypes.windll.shell32", mock_shell32):
    ...
    def test_returns_false_on_attribute_error(self) -> None:
        ...
        with patch("installer.elevation.ctypes.windll", mock_windll):
            assert is_elevated() is False
...
class TestRelaunchElevated:
    def test_exits_on_success(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            patch("installer.elevation.ctypes.windll.shell32", mock_shell32),
            patch("installer.elevation.sys.argv", ["test.exe", "--flag"]),
            patch("installer.elevation.sys.exit", fake_exit),
    ...
    def test_raises_on_failure(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            patch("installer.elevation.ctypes.windll.shell32", mock_shell32),
            patch("installer.elevation.sys.argv", ["test.exe"]),
